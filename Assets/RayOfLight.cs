...
public class RayOfLight : MonoBehaviour {
  ...
  private GameObject closestPrism;
  ...
  private void SetConstants(){
    ...
    computeShader.SetInt( "_Attracting" , 0 );
    computeShader.SetInt( "_NumberHands", handBufferInfo.GetComponent<HandBuffer>().numberHands );
    //computeShader.SetVector( "_PrismPosition" , transform.position);
  ...
  private void Dispatch(){
    ...
    if( closestPrism ){
      //print("Y");
      //print( closestPrism.transform.position );

      //something weird is forcing this hack :/
      computeShader.SetFloat( "_Px" , closestPrism.transform.position.x );
      computeShader.SetFloat( "_Py" , closestPrism.transform.position.y );
      computeShader.SetFloat( "_Pz" , closestPrism.transform.position.z );
    }else{
      computeShader.SetFloat( "_Px" , 10000 );
      computeShader.SetFloat( "_Py" , 10000 );
      computeShader.SetFloat( "_Pz" , 10000 );
    }
  ...
  public void startAttracting( GameObject prism ){
    ...
    computeShader.SetInt( "_Attracting" , 1 );
    closestPrism = prism;
